...
$db = new SQLite3('/xampp/Data/StudentModule.db');
$stmt = $db->prepare('SELECT SumScore, ReviewAmount FROM User WHERE UserName = :username');
$stmt->bindParam(':username', $_SESSION['username'], SQLITE3_TEXT);
$result = $stmt->execute();
$rows_array = [];
while ($row=$result->fetchArray())
...
<div class="row">
  <div class="side" style="margin-left:10px">
    <div>Your Score Average</div>
  </div>
  <div class="middle">
    <div class="bar-container">
      <div style="width:<?php echo $avgscorePercent?>%; height: 15px; background-color: #2196F3;"></div>
