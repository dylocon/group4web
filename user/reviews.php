...
<main class="form-signin col-md-2 center">
  ...
  <table style="width: 100%; text-align: center;border: solid 2px black">
    ...
    $db = new SQLite3('/xampp/Data/StudentModule.db');
    $stmt = $db->prepare("SELECT ReviewID,QuestionID, AppraiseeID  FROM ReviewsQ WHERE AppraiseeID = $userID ");
    $result = $stmt->execute();
    $rows_array = [];
    $count = 0;
    while ($row=$result->fetchArray())
    ...
    <tr>
      <td><a href="userAnswer.php?revieID=<?php echo $rows_array[$x][0]?>"><?php echo $rows_array[$x][0]?></a></td>
