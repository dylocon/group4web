...
<main class="form-signin col-md-2 center">
  ...
  <?php
    ...
    $db = new SQLite3('/xampp/Data/StudentModule.db');
    $stmt = $db->prepare("SELECT AnswerID, ReviewID, AppraiseeID, QuestionID FROM ReviewsA");
    $result = $stmt->execute();
    $rows_array = [];
    $count = 0;
    while ($row=$result->fetchArray())
  ...
  <table style="width: 100%; text-align: center;border: solid 2px black">
    ...
    <tr>
      <td><a href="viewAnswers.php?answersID=<?php echo $rows_array[$x][0]?>"><?php echo $rows_array[$x][0]?></a></td>
  ...
  <h1 style="text-align: center"> You have no reviews. </h1>
  <?php }?>
